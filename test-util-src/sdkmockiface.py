...
class OutWebReqHandle(TracerHandle):
    has_out_tag = True
    is_entrypoint = True
...
class Path(ThreadBoundObject):
    ...
    def add_custom_request_attribute(self, key, value):
        self.nodestack[-1].custom_attribs.append((key, value))
...
def _mk_add_kvs_fn(adder, web_req_handle_type):
...
class SDKMockInterface(object): #pylint:disable=too-many-public-methods
    ...
    incomingwebrequesttracer_add_request_headers = _mk_add_kvs_fn(
        incomingwebrequesttracer_add_request_header, InWebReqHandle)
    ...
    incomingwebrequesttracer_add_response_headers = _mk_add_kvs_fn(
        incomingwebrequesttracer_add_response_header, InWebReqHandle)
    ...
    incomingwebrequesttracer_add_parameters = _mk_add_kvs_fn(
        incomingwebrequesttracer_add_parameter, InWebReqHandle)
    ...
    def outgoingwebrequesttracer_create(self, uri, http_method):
        _strcheck(uri)
        _strcheck(http_method)
        return OutWebReqHandle(self, uri, http_method)

    #pylint:disable=invalid-name

    @_entry_field
    def outgoingwebrequesttracer_add_request_header(self, tracer_h, key, val):
        _livecheck(tracer_h, OutWebReqHandle)
        _strcheck(key)
        _strcheck(val)
        tracer_h.req_hdrs.append((key, val))

    outgoingwebrequesttracer_add_request_headers = _mk_add_kvs_fn(
        outgoingwebrequesttracer_add_request_header, OutWebReqHandle)
    ...
    outgoingwebrequesttracer_add_response_headers = _mk_add_kvs_fn(
        outgoingwebrequesttracer_add_response_header, OutWebReqHandle)
    ...
    def customrequestattribute_add_integer(self, key, value):
        ...
        my_path = self.get_path(True)
        my_path.add_custom_request_attribute(key, value)

    def customrequestattribute_add_float(self, key, value):
        _strcheck(key)
        _typecheck(value, float)
        my_path = self.get_path(True)
        my_path.add_custom_request_attribute(key, value)
    ...
    def customrequestattribute_add_integers(self, keys, values, count):
        ...
        for _, key, val in zip(range(count), keys, values):
            self.customrequestattribute_add_integer(self, key, val)

    def customrequestattribute_add_floats(self, keys, values, count):
        _typecheck(count, int)
        assert count > 0, 'Invalid count'
        for _, key, val in zip(range(count), keys, values):
            self.customrequestattribute_add_float(self, key, val)
    ...
    def trace_in_process_link(self, link_bytes):
        assert link_bytes == b'inproc'
        return InProcessLinkTracerHandle(self)
